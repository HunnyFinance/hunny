...
abstract contract PancakeSwapV2 is OwnableUpgradeable {
    ...
    address internal constant HUNNY = 0xC9849E6fdB743d08fAeE3E34dd2D1bc69EA11a51;
    ...
    function tokenToHunnyBNB(address token, uint amount) internal returns(uint flipAmount) {
        if (token == CAKE) {
            ...
            // Burn HUNNY!!
            IBEP20(HUNNY).transfer(DEAD, amount);
            flipAmount = 0;
        } else {
            // flip
    ...
    function _cakeToHunnyBNBFlip(uint amount) private returns(uint flipAmount) {
        swapToken(CAKE, amount.div(2), HUNNY);
        swapToken(CAKE, amount.sub(amount.div(2)), WBNB);

        flipAmount = generateFlipToken();
    }

    function _flipToHunnyBNBFlip(address flip, uint amount) private returns(uint flipAmount) {
        ...
        if (_token0 == WBNB) {
            swapToken(_token1, IBEP20(_token1).balanceOf(address(this)), HUNNY);
            flipAmount = generateFlipToken();
        } else if (_token1 == WBNB) {
            swapToken(_token0, IBEP20(_token0).balanceOf(address(this)), HUNNY);
            flipAmount = generateFlipToken();
        } else {
            swapToken(_token0, IBEP20(_token0).balanceOf(address(this)), HUNNY);
    ...
    function generateFlipToken() private returns(uint liquidity) {
        uint amountADesired = IBEP20(HUNNY).balanceOf(address(this));
        uint amountBDesired = IBEP20(WBNB).balanceOf(address(this));
        _approveTokenIfNeeded(HUNNY);
        _approveTokenIfNeeded(WBNB);

        (,,liquidity) = ROUTER.addLiquidity(HUNNY, WBNB, amountADesired, amountBDesired, 0, 0, address(this), block.timestamp);

        // send dust
        IBEP20(HUNNY).transfer(msg.sender, IBEP20(HUNNY).balanceOf(address(this)));
